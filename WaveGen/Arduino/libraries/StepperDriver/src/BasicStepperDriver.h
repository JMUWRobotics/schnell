...
public:
    ...
    long calcStepsForRotation(long deg){
        return deg * motor_steps * (long)microsteps * 4 / 360; // times 4 because of 1:4 gear ratio
    }
    long calcStepsForRotation(double deg){
        return deg * motor_steps * microsteps * 4 / 360; // times 4 because of 1:4 gear ratio
    }
};
